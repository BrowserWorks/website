...
} from '~/components/use/clickoutside.types'
...
export const clickoutside: Action<
	HTMLElement,
	Partial<ClickOutsideParameters> | undefined,
	ClickOutsideAttributes
> = function (node, parameters = { enabled: true }) {
	...
	return {
		update(update) {
			...
			;({ enabled, eventType, nodeForEvent, options, capture } = resolveParameters(
				update as Partial<ClickOutsideParameters>
			))
